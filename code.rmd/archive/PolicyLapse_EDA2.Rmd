...
#### Gender and numeric variables

```{r}
policy %>%
  select( Lapsed, PO_Sex, PO_Age, INS_Age, Premium,) %>%
  pivot_longer(where(is.numeric), names_to = "var", values_to = "value") %>%
  ggplot(aes(x=PO_Sex, y=value, fill = Lapsed, color = Lapsed)) +
  geom_boxplot(alpha = 0.7) +
  facet_wrap(~var, scale = "free_y", nrow = 2) +
  labs(y=NULL, color=NULL, fill=NULL)
```

We can see the different after separating out Gender, Lapse status with numeric measure:

- *Premium* : looks no different between Gender, but significant different between Lapse status (both median and variant), so that we can confirm this is an importance variables.
- *PO_Age* : looks like a small different for subset of male gender
- *INS_Age* : interestingly look like they make no diffence
