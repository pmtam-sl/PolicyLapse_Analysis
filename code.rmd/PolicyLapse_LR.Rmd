...
$$
P(y|x)=\frac{e^{𝜷_0+𝜷_1x}}{1+e^{𝜷_0+𝜷_1x}} (1)
$$
...
$$
OR(y)=\frac{P(y|x)}{1-P(y|x)}=e^{𝜷_0+𝜷_1x} (2)
$$
...
$$
log(\frac{P(y|x)}{1-P(y|x)})=𝜷_0+𝜷_1x (3)
$$

Logistic regression transforms as a linear regression to find the log-odds of the probability. By analyzing linear regression function in formula (3) we can find the intercept($𝜷_0$) and coefficient ($𝜷_1$) in order to solve Logistic regression in formula (2) and (1) as well as find their Odds ratio $OR(y)$ and Probability $P(y)$ .

With result, we can interpret: Increasing x by one unit changes

a.  the log odds by $𝜷_1$, or
b.  the OR(y) by $e^{𝜷_1}$, or
c.  the P(y) by $\frac{e^{𝜷_1}}{1+e^{𝜷_1}}$
...
```{r, include = FALSE}
# Kiểm tra dataset - To be deleted
...
# PO_Occupation có p<0.05 có ý nghĩa thống kê 
...
#predictors1 = Occ2[,2:6] # lấy lại ma trận txt từ cột 2 đến 5 - loại trừ Lapse 
#outcome1 =Occ2[,1] # Gán biến outcome
...
# Since Premium is Character, we need to convert
...
# Deviance : khác biệt giá tri quan sát và giá trị tiên lượng 
# Deviance = y-p(x) (y giá trị quan sát , p(x) giá tri tiên lương với independet variables (x))
# Deviance = -2 x log likelihood (or Residual Deviance)
deviance(glm2);deviance(glm1)
# Nul Deviance : Mô hình chưa có independent variables
# Residual Deviance : Mô hình đã có independent variables (predictor variables)
# Khác biệt càng lớn giữa Nul Deviance và Residual Deviance chứng tỏ tác động của idependent variable vô mô hình càng lớn.

# Xem xét các chỉ số chỉ độ thích hợp của mô hình ("Goodness of fit")
# Model Likelihood Ratio Test
# LR chi2 : LRT chi-square 

# Discrimination Indexes (chỉ số liên quan đến phân định )

# R2 = R-square , còn gọi là Hệ số xác định Coefficient of Determination cung cấp chì số nếu mô hình thích hợp hay không ?
# Trong mô hìh hồi quy tuyên tính R2 = 1- (residual SS/total SS) - 
# Trong mô hình hồi quy logic tuyến tính không có hệ số R2, chỉ có "pseudo-R2"
...
# Chỉ số Brier 
# Có thể xem như "mean square error" - MSE
# Brier càng thấp càng tốt !
# https://rdrr.io/cran/DescTools/man/BrierScore.html
lrm2$stats[11]

# Rank Discrimination . Indexes (chỉ số liên quan đến phân định - có sắp xếp thứ tự)
# C : is AUC - Area Under the Curve trong đường biễu diễn ROC - Receiver Operating Characteristic curve
# Đánh giá độ phân định (discrimination) giửa Laspe và khôg Lapse
...
lrm2$stats[6]
# Có thể tính Xác suất : mà chúng ta quan sát được khi mà giá trị tiên lương của Lapse cao hơn gá trị tiên lương của người không Lapse ???  

# Dxy = hệ số tương quan theo thứ bậc (Somers ranks correlation)
# đo lường độ tương quang giữa xác suất tiên lượng (p) và biến cố quan sát
# Dxy = 2(c-0.5)
lrm2$stats[7]
...
# k là số bậc / hay giá trị của Y (thường là 2)
...
# Xem thêm các ý nghĩa của các thông số trong mô hình 
...
# AUC > 0.8 is considered good model - mô hình có phân định (concordance) tốt 
...
# To use Brier score = residual bình phuong to evaluate 
lrm2$stats[11]
...
# Mo hình fix khá tốt với absolute error = 0.041 nhỏ 
summary(cal)
...
```{r, echo=FALSE}
library(BMA)
predictors2 = dat2[,2:5] # lấy lại ma trận txt từ cột 2 đến 5 - loại trừ Lapse 
outcome2 =dat2[,1] # Gán biến outcome
...
-   For each 1,000 Dollar increase of *Premium*, logit(P) decreases 1,000\*0.000126 units = 0,1261 units, OR(̂P) changes $e^{0.1261}=`r exp(0.1261)`$ or *predict* Odds(P) increase 13.44%
